...
from datasets import build_dataset, build_dataset_blur
...
def get_args_parser():
    ...
    # Instead of resume, add deit_model params - for starting from a trained model:
    parser.add_argument('--deit_model_dir', default='out', type=str, help='directory (within "deit") of deit model')
    parser.add_argument('--deit_model_name', default=None, type=str, help='model name, or None for untrained deit')
...
def main(args):
    ...
    if args.deit_model_name:
        deit_mdl_pth = os.path.join('/home/projects/bagon/ilanaveh/code/Transformers/deit',
                                    args.deit_model_dir, args.deit_model_name, 'checkpoint.pth')
        if os.path.isfile(deit_mdl_pth):
            checkpoint = torch.load(deit_mdl_pth, map_location='cpu')
            print(f"Loading model: {args.deit_model_name} (epoch: {checkpoint['epoch']})")
            model.load_state_dict(checkpoint['model'])
        else:
            print(f"No such model {args.deit_model_name}")
            return None
