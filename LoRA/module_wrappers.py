...
2. ViTWithFeatures: a modified forward method, that enables access to features (used for lora feature-consistency)
"""
...
class ViTWithFeatures(VisionTransformer):
    """
...
def forward_with_features(self, x, return_features: bool = False):
    x = self.forward_features(x)
    feats = self.pool(x)
    feats = self.fc_norm(feats)
    logits = self.head_drop(feats)
